...
transformed parameters {
  ...
  matrix[nSubjects, 3] v = (diag_pre_multiply(tau_v, L_v) * z_v)';
  ...
  vector[nSubjects] d_c = d_gamma + v[,3];
} 
  ...
  for (s in 1:nSubjects) { // for each subject
    for (t in 1:4) { // for each load completed (70%, 80%, 90%, 100% 1-RM)
      for (z in 1:2) { // for each occasion (T1 or T2)
          target += normal_lpdf(Load[s, t, z] |  (c[s] + (d_c[s] * (z-1))) + (a[s] + (d_a[s] * (z-1))) * exp((b[s] + (d_b[s] * (z-1))) * Repetitions[s, t, z]), sigma);
      }
    }
  }
}
